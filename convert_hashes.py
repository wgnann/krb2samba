...
userlist = [(codecs.decode(u[1]["uid"][0]), u[0]) for u in samba_results if "uid" in u[1]]
...
for user in userlist:
    if not user[0] in injson:
        print("No hashes for user " + str(user[0]) + " were found, ignoring.")
    else:
        ...
        if len(set(userprops.keys()) & {"type1", "type3", "type17", "type18"}) != 4:
            ...
            k2sc_popen = subprocess.Popen(["/usr/bin/python3 " + k2sc_path + os.sep + "kerberos2supplementalCredentials.py --base64 " + userprops["salt"] + " " + k2sc_params],
                    shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            supplementalCredentials = k2sc_popen.stdout.readlines()
            if (not all((d in string.ascii_letters or d in string.digits or d in "+/=\n") for d in codecs.decode(supplementalCredentials[0]))):
                sys.exit("kerberos2supplementalCredentials.py error:\n" + "".join(supplementalCredentials))
            addModify(user[1], "supplementalCredentials", codecs.decode(supplementalCredentials[0]).replace("\n", ""), True)
...
print("The control 1.3.6.1.4.1.7165.4.3.12 enables editing of the unicodePwd and supplementalCredentials attributes.")
